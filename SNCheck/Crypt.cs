...
namespace aat
{
    class Crypt
    {
        ...
        /// <returns></returns>
        public static bool makeKeyFile(string keyFileName, string userName, string companyName, string validDate, string UID)
        {
            bool rtn = false;
            try
            {
                ...
                using (StreamWriter sw = new StreamWriter(keyFileName, false, Encoding.UTF8))
                {
                    ...
                    sw.WriteLine("Company Name:" + companyName);
                    sw.WriteLine("Validation Date:" + validDate);
                    sw.WriteLine("Key:" + AESDecrypt(validDate));
                    sw.WriteLine("License:" + AESDecrypt(MyHash(UID)));
                    sw.Close();
                }
        ...
        private static DateTime getKeyTM()
        {
            ...
            try
            {
                ...
                using (StreamReader sr = new StreamReader(keyFile, Encoding.UTF8))
                {
                    ...
                    {
                        string d = sr.ReadLine();
                        if (d.StartsWith("Validation Date:"))
                        {
                            tm = d.Substring(16);
                        }
                        else if (d.StartsWith("Key:"))
                        {
                            if (AESDecrypt(d.Substring(4)) == tm)
                            {
